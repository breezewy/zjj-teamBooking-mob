;(function($){
    ...
    init : function (_options) {
      return this.each(function () {
        ...
        $this.on("click","td:not(.other,.prevM,.nextM)", function() {
          ...
          if(opts.multiselect){
            var selecteddates=$this.find("td.selected"),prices=[];
            if(selecteddates.length==2){
              selecteddates.removeClass('selected').removeData("selected").find(".calendar-explian").html('');
            ...
            if(!private_methods.duibi(onedate,date)){
              multiselectflag=1;
              var price=$this.find("td.selected").find(".calendar-explian").data("price");
              if(typeof price=="undefined"){
                $this.find("td.selected").find(".calendar-explian").html("");
              }else{
                $this.find("td.selected").find(".calendar-explian").html("<span class='price'><em>￥</em><strong>"+price+"</strong></span>");
              }
            ...
            if(private_methods.duibi(onedate,date)==="0"){
              multiselectflag++;
            }
            ...
            if(multiselectflag==1){
              oldprice=$(this).find("strong").text();
              $(this).find(".calendar-explian").html("入住");
            }
            if(multiselectflag==2){
              prices.push(oldprice);
              $("#calendar").find("td.selecteds").each(function(){
                prices.push($(this).find("strong").text());
              });
              prices.push($(this).find("strong").text());
              $(this).find(".calendar-explian").html("离店");
              opts.onClick(this,[onedate,date],prices);
            }
          }
          else{
            $this.find("td").removeClass('selected');
            $(this).addClass('selected');
            opts.onClick(this,date,price);
          }
        });
      });
    },
  ...
  var private_methods = {
    ...
    price:function(date,opts){
      var pricetext="",show=false;
      //console.log(opts.optionsdays);
      if(opts.optionsdays){
        for(var i= 0;i<opts.optionsdays.length;i++){
          var targetdate=opts.optionsdays[i].date;
          var price=typeof opts.optionsdays[i].price=="undefined"?"":opts.optionsdays[i].price;
          if(date==targetdate){
            pricetext+="<p class='calendar-explian' data-price='"+price+"'><span class='price' data-ticket='"+opts.optionsdays[i].leftNum+"' data-kid='"+opts.optionsdays[i].kid+"' >";
            if(price!=""){
              pricetext+="<em>￥</em><strong>"+price+"</strong>";
            }
            pricetext+="</span></p>";
            show=true;
          }
        ...
        pricetext="<p class='calendar-explian'></p>";
      }
  ...
  var calendar = {
    ...
    }).height($(window).height()-($(window).width()/640)*40*2.5),
    dayHtml: function() {
      // var html = "<tr>";
    ...
    dateHtml: function(year, month,opts) {
      ...
      for (var i = 0; i < presentDate.getDay(); i++) {
        ...
        if(opts.optionsdays){
          html += "<td ";
          if($.inArray(previtemDay,private_methods.settingdate(opts.optionsdays))==-1){
            html+=" class='other'";
          }
          html+=" data='prevM'>";
        }
      ...
      for (var j = 0; j < days; j++) {
        ...
        html+="<td ";
        if(today == itemDay){
          //if($.inArray(today,private_methods.settingdate(opts.optionsdays))>-1){
          if(typeof opts.selecteday=="string"&&opts.selecteday==itemDay){
            html += " class='selected'>";
            html+="<span class='date-title'>今天</span>"+pricedata[0]+"";
          }
          else if(opts.selecteday[0]==itemDay){
            html += " class='selected'>";
            html+="<span class='date-title'>今天</span><p class='calendar-explian'>入住</p>";
          }
          else if(opts.selecteday[1]==itemDay){
            html += " class='selected'>";
            html+="<span class='date-title'>今天</span><p class='calendar-explian'>离店</p>";
          }
          else{
            if(opts.optionsdays&&$.inArray(today,private_methods.settingdate(opts.optionsdays))==-1){
              html += " class='other'>";
            }else{
              html += ">";
            }
          ...
          html += " class='selected'><span class='date-title'>" + (j + 1) + "</span>"+pricedata[0];
        }
        else if(opts.selecteday[0]==itemDay){
          html += " class='selected'><span class='date-title'>" + (j + 1) + "</span><p class='calendar-explian'>入住</p>";
        }
        else if(opts.selecteday[1]==itemDay){
          html += " class='selected'><span class='date-title'>" + (j + 1) + "</span><p class='calendar-explian'>离店</p>";
        }
        else{
          if(opts.optionsdays){
            if($.inArray(itemDay,private_methods.settingdate(opts.optionsdays))==-1){
              html+=" class='other'";
          ...
          html += "><span class='date-title'>" + (j + 1) + "</span>"+pricedata[0];
        }
        html += "</td>";
      ...
      for (var k = 0; k < 42 - presentDate.getDay() - days; k++) {
        ...
        if(opts.optionsdays){
          html += "<td class='other'";
          if($.inArray(nextitemDay,private_methods.settingdate(opts.optionsdays))==-1){
            //html+=" class='other'";
          }
          html+=" data='nextM'>";
        }
